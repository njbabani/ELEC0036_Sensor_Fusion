# RECORD ACCELEROMETER MEASUREMENTS OF BNO085
...
FILENAME = os.path.join(os.getcwd(), 'BNO085_linear_accel_data_cal.txt')  # Output file
    ...
    for _ in range(max_record):
        try:
            ...
            if len(data) >= 1:
                ...
                # raw_accel = float(data[0])
                # cal_accel = float(data[1])
                # accel_list.append([raw_accel, cal_accel])
                # imu_accel = float(data[0])
                # accel_list.append([imu_accel])
                raw_accel = float(data[0])
                accel_list.append([raw_accel])
            else:
...
def main():
    ...
    for _ in range(TOTAL_MEAS):
        ...
        if user == 'm':
            ...
            accel_list = record_data(ser)
            data.extend(accel_list)
            print('[INFO]: Acceleration (m/s^2) = {}'.format(accel_list))
        elif user == 's':
