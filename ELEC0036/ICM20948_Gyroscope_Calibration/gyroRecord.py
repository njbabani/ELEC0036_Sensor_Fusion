...
#          BABANZ          #
############################
# RECORD GYROSCOPE MEASUREMENTS FOR CALIBRATION
...
# Global variables
MAX_AVG_MEAS = 100  # Maximum number of average measurements in the session, so that we don't create an infinite loop
...
FILENAME = os.path.join(os.getcwd(), 'calgyrodata.txt')  # Output file
...
def recordData(ser: SerialPort):
    # Initialise gx, gy, and gz as 0.0 for float values
    gx = gy = gz = 0.0
    ...
    return (gx / AVG_MEAS, gy / AVG_MEAS, gz / AVG_MEAS)
    ...
    for _ in range(MAX_AVG_MEAS):
        ...
        if user == 'm':
            ...
            for _ in range(MAX_RECORD):
                gx, gy, gz = recordData(ser)
                print('[INFO]: Avgd Readings: {:.4f}, {:.4f}, {:.4f}'.format(gx, gy, gz))
