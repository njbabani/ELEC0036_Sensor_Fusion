...
# Global variables
TOTAL_MEAS = 5  # Maximum number of measurements in the session, to prevent an infinite loop
...
FILENAME = os.path.join(os.getcwd(), 'magdata.txt')  # Output file
    ...
    for _ in range(max_record):
        try:
            ...
            if len(data) >= 3:
                ...
                magy = float(data[1])
                magz = float(data[2])
                mag_list.append((magx, magy, magz))
            else:
...
def main():
    ...
    for _ in range(TOTAL_MEAS):
        ...
        if user == 'm':
            ...
            mag_list = recordData(ser)
            data.extend(mag_list)
            print('[INFO]: Magnetometer readings (uT) = {}'.format(mag_list))
        elif user == 's':
