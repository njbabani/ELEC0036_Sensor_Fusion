...
const float outlier_threshold_y = 6.0;                    // Outlier threshold for y for rejection purposes
...
// Define variables (PF)
char FILE_NAME[] = "/PF_FC132_Final.txt";  // Name of the file written to SD card
struct Particle {                          // Define a structure for particles
  float x;
  float y;
  float weight;
};
float x_hat = 0;                        // Estimated x-position
float y_hat = 0;                        // Estimated y-position
float likelihood_coeff;                 // Initialise the likelihood coefficient
const float user_x0 = 3.50;             // Initial user x0
const float user_y0 = 0.40;             // Initial user y0
const float initial_radius = 5.0;       // Specify radius to initialise particles with specified radius
const float sensor_noise_std_dev = 20;  // Noise for the likelihood function
const int measurement_max = 200;        // Maximum amount of measurements
const int NUM_PARTICLES = 5000;         // Define the number of particles
Particle particles[NUM_PARTICLES];      // Initialise particles array
Particle new_particles[NUM_PARTICLES];  // Initialise resampling particle array

// Define flags (IMU)
bool step_detect_flag = false;   // Indicates if valid step has been detected, if so then updates user position
bool step_detect_reset = false;  // This step detection flag
...
// Define flags (PF)
bool predict_pf_flag = false;
bool update_pf_flag = false;
...
// User position is now handled inside particle filter prediction
void PDR(void) {
  if (step_detect_flag == true) {
    // Reset the step detection flag
    step_detect_flag = false;
    predict_pf_flag = true;
  }
}
...
void scanTask(void* pvParameters) {
  BLEDevice::init("");                       // Initialise the device
...
// Initialise particles radially
void initPFParticles(float x, float y, float r) {
  for (int i = 0; i < NUM_PARTICLES; i++) {
    float angle = random(0, 360) * PI / 180.0;  // Random angle in radians
    float radius = random(0, r) / r;            // Random radius from 0 to input r

    // Convert polar to Cartesian coordinates
    particles[i].x = user_x0 + radius * cos(angle);
    particles[i].y = user_y0 + radius * sin(angle);
    particles[i].weight = 1.0 / NUM_PARTICLES;  // Initialise weights equally
  }
}

// Propagate particles
void predictPF(void) {
  if (predict_pf_flag == true) {
    for (int i = 0; i < NUM_PARTICLES; i++) {
      particles[i].x += lambda * cos(psi * DEG_TO_RAD);
      particles[i].y += lambda * sin(psi * DEG_TO_RAD);
    }
  }
}

// Esimate the position from particle filter
void estimatePF(void) {
  x_hat = 0.0;
  y_hat = 0.0;
...
// Updates the PF dependent on measurements
void updatePF(float measurement_x, float measurement_y) {
  if (update_pf_flag == true) {
...
// Gaussian measurement likelihood function using Euclidean distance
...
// Systematically resamples the particles
...
void setup(void) {
  ...
  // Initialise PRNG with random seed
...
void loop() {
  ...
  // Check IMU sensors for steps and heading
  if (sensorValue.sensorId == SH2_ACCELEROMETER) {
    ...
    if (predict_pf_flag == true) {
      ...
      saveData(x_hat, y_hat, t_exp);

      // Deassert flag for next measurement
      predict_pf_flag = false;
  ...
  if (xSemaphoreTake(mutex, (TickType_t)10) == pdTRUE) {
    ...
    if (update_pf_flag == true) {
      // Compute least squares trilateration and update particles with measurement
      updatePF(lsq_x, lsq_y);
