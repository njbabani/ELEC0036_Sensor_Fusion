...
FILENAME = os.path.join(os.getcwd(), 'tp_fast_10.txt')  # Output file
    ...
    for _ in range(max_record):
        try:
            ...
            if len(data) >= 1:
                ...
                # filt_accel = float(data[1])
                # linear_accel_list.append([raw_accel, filt_accel])
                filt_accel = float(data[0])
                linear_accel_list.append(filt_accel)
            else:
