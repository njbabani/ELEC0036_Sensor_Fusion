# RECORD RSSI MEASUREMENTS OF RASPBERRY PICO ADVERTISEMENTS FROM ARDUINO NANO ESP32
...
# Global variables
TOTAL_MEAS = 41  # Maximum number of measurements in the session, to prevent an infinite loop
...
FILENAME = os.path.join(os.getcwd(), 'RSSI_tp_node_3.txt')  # Output file
...
def record_data(ser: SerialPort, max_record: int = MAX_RECORD):
    ...
    for _ in range(max_record):
        try:
            data = ser.read().split(',')
            if len(data) >= 2:
                RSSI_now = float(data[0])
                RSSI_filtered = float(data[1])  # Use square brackets to access list elements
                RSSI_list.append(RSSI_now)
                RSSI_list.append(RSSI_filtered)
            else:
